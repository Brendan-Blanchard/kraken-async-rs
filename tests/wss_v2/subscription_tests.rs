...
mod execution_subscription {
    ...
    fn get_expected_execution_subscription() -> Value {
        json!({"method":"subscribe","params":{"channel":"executions","token":"someToken","snapshot_trades":true,"snapshot":true,"snap_orders":true,"snap_trades":false},"req_id":0})
    }
    ...
    async fn test_execution_subscription() {
        ...
        execution_params.snap_trades = Some(false);
        execution_params.snap_orders = Some(true);
